        ...
        <MotionStagger className="grid gap-5 md:grid-cols-2" delay={0.08}>
          {benefits.map((item) => (
            <MotionItem key={item.title}>
              <HoverLift>
                <div className="panel hover-panel shine-border p-6">
                  <div className="flex items-center justify-between gap-4">
                    <item.icon className="h-6 w-6 text-gold" />
                    <ArrowUpRight className="h-4 w-4 text-white/30 transition-transform duration-500 group-hover:translate-x-1" />
                  </div>
